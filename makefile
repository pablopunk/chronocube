...
osx:
	...
	cd src && electron-packager . Chronocube --platform darwin --arch=all --version=0.5.1 --icon=img/icon.png --out=../build --overwrite --ignore=node_modules && cd ..
...
win:
	...
	cd src && electron-packager . Chronocube --platform win32 --arch=ia32 --version=0.5.1 --icon=img/icon.png --out=../build --overwrite --ignore=node_modules && cd ..
...
linux32:
		cd src && electron-packager . Chronocube --platform linux --arch=ia32 --version=0.5.1 --icon=img/icon.png --out=../build --overwrite --ignore=node_modules
		cd build && zip -r ../dist/Chronocube-Linux32.zip Chronocube-linux-ia32

linux64:
		cd src && electron-packager . Chronocube --platform linux --arch=x64 --version=0.5.1 --icon=img/icon.png --out=../build --overwrite --ignore=node_modules
