...
osx:
	rm -rf dist/osx
	cd src && electron-packager . Chronocube --platform darwin --arch=all --version=0.30.1 --icon=img/icon.png --out=../build --overwrite && cd ..
...
linux32:
		cd src && electron-packager . Chronocube --platform linux --arch=ia32 --version=0.30.1 --icon=img/icon.png --out=../build --overwrite
		cd build && zip -r ../dist/linux32.zip Chronocube-linux-ia32

linux64:
		cd src && electron-packager . Chronocube --platform linux --arch=x64 --version=0.30.1 --icon=img/icon.png --out=../build --overwrite
